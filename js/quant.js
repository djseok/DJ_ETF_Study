...
let globalFuturesDelta = 0; // 🌟 신규: 나스닥 선물 실시간 변동률
...
function extractGlobalMacroVariables() {
    ...
    macroData.forEach(row => {
        ...
        // 🌟 1. [진짜 실시간 나스닥 선물] - 티커가 '나스닥선물지수'이거나 종목명이 '크롤링'인 경우
        ...
        // 🌟 2. [나스닥 본장 지수 전광판] - 본장 지수(.IXIC) 또는 NDX인 경우
        else if (ticker === '나스닥' || ticker === '나스닥선물' || name === 'NDX' || name === '.IXIC' || combinedText.includes('IXIC') || combinedText.includes('NDX')) {
            ...
            if(document.getElementById('macro-fx')) {
            ...
            globalVixValue = live;
            if(document.getElementById('macro-vix')) {
            ...
            if(document.getElementById('macro-tnx')) {
